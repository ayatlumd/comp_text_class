...
```   
Aigerim (Aya) Toleukhanova, Ph.D. student
aigerimt@umd.edu 
ver. 9/02/2026
```   
...
-   Understand basic operations of R Studio
...
-   Create a new folder on your hard drive: Code
-   Create a subfolder: comp_text_class
-   Download intro_to_r.zip and move to comp_text_class
-   Unzip file, open code.Rproj
...
dplyr is loaded when you call the tidyverse library
...
from 52788 to 69982
2: Number of rows  4
3: Number of rows cut with filter of greater that 500 = 0?
```
...
#Which one is higher? There is no company under name "Business", only "Business Insider" and Independent Business and etc. So Bloomberg has higher salaries.
...
3: Find highest paid editor. Resent them.   ran above - CondeNast editorial director
...
```{r}
Photographers <- subset(MediaBucks, grepl("?photographer", TITLE))
summary(Photographers$Salary)
